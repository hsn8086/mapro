...
| `sharedTrack` | `string` | (可选) 共轨伙伴线路 ID（由该线路承载共轨段的几何路径）。当相邻两个 LineStation 都声明同一伙伴时，两站之间的区段视为共轨：地图上只画一条轨道，颜色在两条线之间等宽交替（条纹与轨道同宽，本线未开通段用状态灰色），两线地位平等、互不从属；本线不再单独占用并线槽位，站点徽章仍按本线状态显示。伙伴线路必须实际经过该区段，否则标记不生效 |
