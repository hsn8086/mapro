...
| `sharedTrack` | `string` | (可选) 共轨宿主线路 ID。当相邻两个 LineStation 都声明同一宿主时，两站之间的区段视为共轨：本线不再单独占用并线槽位，而是以本线颜色（未开通段为状态灰色）的短划线叠加在宿主线路轨道上，表示两线共用一条轨道；站点徽章仍按本线状态显示。宿主线路必须实际经过该区段，否则标记不生效 |
