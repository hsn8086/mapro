...
| `sharedTrack` | `string` | (可选) 共轨伙伴线路 ID（由该线路承载共轨段的几何路径）。当相邻两个 LineStation 都声明同一伙伴时，两站之间的区段视为共轨：地图上只画一条轨道宽度，两条线各占一半宽度并排（本线以半宽描边贴在伙伴轨道一侧，未开通段用状态灰色，拐角保持同心圆弧），两线地位平等、互不从属；本线不再单独占用并线槽位，站点徽章仍按本线状态显示。伙伴线路必须实际经过该区段，否则标记不生效 |
